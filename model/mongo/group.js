...
Group.prototype.edit = function (data, cb) {
  ...
  __self.validateId(data.id, (err, _id) => {
    ...
    if (data.packages) {
      for (let i = 0; i < data.packages.length; i++) {
        let prodPack = data.packages[i];
        s.$set['config.allowedPackages.' + prodPack.product] =
          prodPack.packages;
      }
    }
...
Group.prototype.updateEnvironments = function (data, cb) {
  ...
  if (
    !data ||
    !(data.environments && Array.isArray(data.environments)) ||
    (!data.groups && !(data.groups.ids || data.groups.codes))
  ) {
    let error = new Error(
      'Group: environments and group codes or ids are required.'
    );
    return cb(error, null);
  }
  let s = {
...
Group.prototype.updatePackages = function (data, cb) {
  ...
  if (
    ...
    let error = new Error(
      'Group: packages and group codes or ids are required.'
    );
    return cb(error, null);
  }
  let s = {
...
Group.prototype.deleteEnvironments = function (data, cb) {
  ...
  if (
    ...
    let error = new Error(
      'Group: id or code in addition to environment(s) are required.'
    );
    return cb(error, null);
  }
...
Group.prototype.deleteProducts = function (data, cb) {
  ...
  if (
    ...
    let error = new Error(
      'Group: id or code in addition to products are required.'
    );
    return cb(error, null);
  }
  let remove = (condition) => {
